...
class SALibSensitivityAnalysis(CalibrationWorkflowBase):
	...
	def analyze(self) -> None:
		...
		if outdir is not None:
			outfile = self.join(
				outdir, f"{time_now}-{task}-{experiment_name}_indices.png"
			)
		...
		if outdir is not None:
			outfile = self.join(
				outdir, f"{time_now}-{task}-{experiment_name}_heatmap.png"
			)
			self.append_artifact(outfile)
			plt.savefig(outfile)
		else:
		...
		def recursive_write_csv(dfs: pd.DataFrame) -> None:
			if isinstance(dfs, list):
				...
				outfile = self.join(
					outdir, f"{time_now}-{task}-{experiment_name}_{si_type}.csv"
				)
				self.append_artifact(outfile)
		...
		if isinstance(si_dfs, list):
			...
			outfile = self.join(
				outdir, f"{time_now}-{task}-{experiment_name}_{sampler_name}.csv"
			)
			self.append_artifact(outfile)
			si_df.to_csv(outfile, index=False)
